...
{ 
 ...
 public:  // available to the client
   ...
   //          otherwise, throws an exception - Underflow
   // PARAMETER: provide variable to receive the top element of the stack
   void topElem(el_t&);
  
   // ** Must add good comments for each function - See Notes1B

   //PURPOSE: to chack if the stack is empty
   bool isEmpty();

   //PURPOSE: to check is the stack is full
   bool isFull();

   //PURPOSE: display all elements with no change to the stack
   void displayAll();

   //PURPOSE: completely empties the stack
   void clearIt();
