...
{ 
  ...
  catch(llist::OutOfRange){ cerr << "Error has occured - Type is: OutOfRange" << endl; }   // do not exit

  // 7. insert the 0th (element 0) error (out of range)
  cout << "Step" << c << " insert 0th" << endl; c++;
  try{ L.insertIth(0, 0); } 
  catch(llist::OutOfRange){ cerr << "Error has occured - Type is: OutOfRange" << endl; } 
  // do not exit
  ...
  try { L.deleteIth(5, x); }
  catch(llist::OutOfRange){ cerr << "Error has Occured - Type is: OutOfRannge" << endl; }
  ...
  try { L.deleteFront(x); }
  catch(llist::Underflow){ cerr << "Error has Occured- Type is: Underflow" << endl; } 
