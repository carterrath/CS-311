...
void BST::DeleteVertex(el_t E)
{
  ...
  { cout << "... deleting the root with just one child" << endl; 
    if (Root->Right == NULL && Root->Left != NULL) // and it has only the left subtree
      { Parent = Root; // change the root to the left child and return
	Root = Root->Left;// making sure the old root has been deleted and the new root's UP is NULL
	delete Parent;
	Root->Up = NULL;
	return;
    }
    else if (Root->Left == NULL && Root->Right != NULL) // and it has only the right subtree
      { Parent = Root; // change the root to the right child and return
	Root = Root->Right;// making sure the old root has been deleted and the new root's UP is NULL
	delete Parent;
	Root->Up = NULL;
	return;
    }
  ...
  if (V->Left == NULL && V->Right == NULL) // if V is a leaf (case 1)
    {
     ...
     { 
       ...
       if (C->Elem < P->Elem) { P->Left = C; } // You need if then else to determine Parent's left or right
       else { P->Right = C; }//    should point to C; 
